...
const SNAP: Record<SheetState, number> = { peek: 120, half: 420, full: 740 }
...
export default function BottomSheet({ state, onStateChange, entries, onSelectEntry }: BottomSheetProps) {
  ...
  function snapTo(newState: SheetState) {
  ...
  function onDragEnd() {
    const current = y.get()
    const distances = (Object.entries(SNAP) as [SheetState, number][]).map(
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div style={{ padding: '4px 24px 12px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexShrink: 0 }}>
        <div>
      ...
      {/* Peek: single preview row */}
      {isPeek && entries[0] && (() => {
        const { day, dowShort } = formatDateShort(entries[0].date)
        return (
          <div
            onClick={() => snapTo('half')}
            style={{ padding: '0 24px', display: 'flex', gap: 14, alignItems: 'center', flex: 1, cursor: 'pointer' }}
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {entries[0].mood && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <MoodGlyph mood={entries[0].mood as NonNullable<Mood>} size={12} />
                  <span style={{ fontFamily: 'var(--font-ui)', fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--ink-faded)' }}>
                    {entries[0].mood}
                  </span>
                </div>
              )}
              <div style={{ marginTop: 4, fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--ink-soft)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {entries[0].body}
              </div>
            </div>
            <Icon name="arrowUp" size={18} color="var(--ink-faded)" />
          </div>
        )
      })()}
